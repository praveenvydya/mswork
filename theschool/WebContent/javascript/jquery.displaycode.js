 ...
 * Examples and documentation at: http://demo.webcodingstudio.com/displaycode/
 ...
 */
(function($){
	$.fn.displaycode = function(element_params){
		...
		var defaults = {
			...
			list: 'ol',
			//name of the tag attribute to add a special language displaycodeing
			attribute: 'lang'
		}
		...
		return this.each(function(){
			...
			$(code_container).wrap('<div class="displaycode'+code_lang_class+'"></div>');
			var displaycode_container = $(code_container).parent();
			...
			switch(code_lang) {
				case 'html':
					code = $.displaycodeCode.displaycode_html(code);	
					break;
				case 'css':
					code = $.displaycodeCode.displaycode_css(code);	
					break;
				case 'php':
					code = $.displaycodeCode.displaycode_php(code);	
					break;
				case 'sql':
					code = $.displaycodeCode.displaycode_sql(code);	
					break;
				default:
					code = $.displaycodeCode.displaycode(code);	
					break;
			}
			...
			//init tabs
			if(params.source==true) {
				var tabs = $(displaycode_container).find('ul.tabs li');
				$.each(tabs , function(i,tab){
					$(tab).click(function() {
						$(tabs).removeClass('active');
						var tab_class = $(tab).attr('class');
						$(displaycode_container).find('pre[class!="'+tab_class+'"]').css('display', 'none');
						$(displaycode_container).find('pre[class^="'+tab_class+'"]').css('display', 'block');
						$(tab).addClass('active');
					});
				});
			...
			if(params.zebra==true) {
				$(displaycode_container).find('pre[class="'+code_class+'"] '+params.list+' li:even').addClass('even');
			}
		});
	};
	...
	$.displaycodeCode = {
		...
		//HTML
		displaycode_html: function(code) {
